...
class SoftmaxHighResolutionHead(nn.Module):
    def __init__(self, backbone_channels, num_outputs):
        super(SoftmaxHighResolutionHead, self).__init__()
        last_inp_channels = sum(backbone_channels)
        self.last_layer = nn.Sequential(
            nn.Conv2d(
                in_channels=last_inp_channels,
...
def hrnet_w18(pretrained=False):
    import yaml
    hrnet_dir = '/root/data1/wzj/MultitaskFundusSegementation/models/hrnet'
    hrnet_cfg = os.path.join(hrnet_dir, 'hrnet_w18.yml')
    ...
    if pretrained:
        pretrained_weights = os.path.join(hrnet_dir,'hrnet_w18_small_model_v2.pth')
